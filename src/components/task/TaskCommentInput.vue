...
<script setup>
    ...
    try {
        const res = await axios.get(`/api/mention/search?projectId=1&mentionTarget=${keyword}`, {
        headers: {
...
const fetchDetailList = async (keyword) => {
    try {
        const res = await axios.get(`/api/mention/detail?projectId=1&taskId=${props.taskId}&mentionTarget=${keyword}`, {
        headers: {
            Authorization: `Bearer ${userStore.accessToken}`
        }
        })
    ...
    const selectUser = (user) => {
        ...
        const newBefore = beforeCursor.replace(/[@#][^\s@#]*$/, `${user.type === 'DETAIL' ? '#' : '@'}${user.name} `)
    ...
    if (file) console.log('선택한 파일:', file)
    }
